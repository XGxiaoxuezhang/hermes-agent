...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("new", "开始新会话（新的 session ID 和历史记录）", "Session",
               aliases=("reset",), args_hint="[name]"),
    CommandDef("topic", "启用或查看 Telegram DM topic sessions", "Session",
               gateway_only=True, args_hint="[off|help|session-id]"),
    CommandDef("clear", "清屏并开始新会话", "Session",
               cli_only=True),
    CommandDef("redraw", "强制重绘界面（用于恢复终端显示错位）", "Session",
               cli_only=True),
    CommandDef("history", "查看对话历史", "Session",
               cli_only=True),
    CommandDef("save", "保存当前对话", "Session",
               cli_only=True),
    CommandDef("retry", "重试上一条消息（重新发送给 Agent）", "Session"),
    CommandDef("undo", "移除上一轮用户/助手对话", "Session"),
    CommandDef("title", "设置当前会话标题", "Session",
               args_hint="[name]"),
    CommandDef("handoff", "将当前会话交接到消息平台（Telegram、Discord 等）", "Session",
               args_hint="<platform>", cli_only=True),
    CommandDef("branch", "从当前会话创建分支（探索另一条路径）", "Session",
               aliases=("fork",), args_hint="[name]"),
    CommandDef("compress", "手动压缩对话上下文", "Session",
               args_hint="[focus topic]"),
    CommandDef("rollback", "列出或恢复文件系统 checkpoints", "Session",
               args_hint="[number]"),
    ...
    CommandDef("deny", "拒绝待处理的危险命令", "Session",
               gateway_only=True),
    CommandDef("background", "在后台运行一个 prompt", "Session",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "显示活跃 agents 和运行中的任务", "Session",
               aliases=("tasks",)),
    CommandDef("queue", "把 prompt 加入下一轮队列（不中断当前任务）", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "在下一次工具调用后插入消息（不中断）", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "设置持续目标，Hermes 会跨轮执行直到完成", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("subgoal", "为当前目标添加或管理额外条件", "Session",
               args_hint="[text | remove N | clear]"),
    CommandDef("status", "查看会话信息", "Session"),
    CommandDef("whoami", "查看你的 slash command 权限（admin / user）", "Info"),
    CommandDef("profile", "显示当前 profile 名称和 home directory", "Info"),
    CommandDef("sethome", "将当前聊天设为 home channel", "Session",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "恢复之前命名的会话", "Session",
               args_hint="[name]"),

    # Configuration
    CommandDef("sessions", "浏览并恢复历史会话", "Session"),
    ...
    CommandDef("codex-runtime", "切换 OpenAI/Codex 模型的 codex app-server runtime",
               "Configuration", aliases=("codex_runtime",),
               args_hint="[auto|codex_app_server]"),
    CommandDef("gquota", "查看 Google Gemini Code Assist quota 用量", "Info",
               cli_only=True),

    CommandDef("personality", "设置预定义 personality", "Configuration",
               args_hint="[name]"),
    ...
    CommandDef("footer", "切换最终回复中的 gateway runtime metadata footer",
               "Configuration", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "切换 YOLO 模式（跳过所有危险命令审批）",
               "Configuration"),
    CommandDef("reasoning", "管理 reasoning effort 和显示方式", "Configuration",
    ...
    CommandDef("busy", "控制 Hermes 忙碌时按 Enter 的行为", "Configuration",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Tools & Skills
    CommandDef("tools", "管理 tools：/tools [list|disable|enable] [name...]", "Tools & Skills",
               args_hint="[list|disable|enable] [name...]", cli_only=True),
    CommandDef("toolsets", "列出可用 toolsets", "Tools & Skills",
               cli_only=True),
    CommandDef("skills", "搜索、安装、查看或管理 skills",
               "Tools & Skills", cli_only=True,
               subcommands=("search", "browse", "inspect", "install", "audit")),
    CommandDef("bundles", "列出 skill bundles（/<name> 可作为多个 skills 的别名）",
               "Tools & Skills"),
    CommandDef("cron", "管理定时任务", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "后台 skill 维护（status、run、pin、archive、list-archived）",
               "Tools & Skills", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived")),
    CommandDef("kanban", "多 profile 协作看板（tasks、links、comments）",
               "Tools & Skills", args_hint="[subcommand]",
    ...
    CommandDef("reload", "将 .env 变量重新加载到运行中的会话", "Tools & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "从配置重载 MCP servers", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "重新扫描 ~/.hermes/skills/ 中新增或移除的 skills",
               "Tools & Skills", aliases=("reload_skills",)),
    CommandDef("browser", "通过 CDP 将浏览器工具连接到当前 Chromium 系浏览器", "Tools & Skills",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "列出已安装 plugins 及其状态",
               "Tools & Skills", cli_only=True),

    # Info
    CommandDef("commands", "分页浏览所有 commands 和 skills", "Info",
               gateway_only=True, args_hint="[page]"),
    CommandDef("help", "显示可用命令", "Info"),
    CommandDef("restart", "等待活跃任务结束后优雅重启 gateway", "Session",
               gateway_only=True),
    CommandDef("usage", "查看当前会话 token 用量和 rate limits", "Info"),
    CommandDef("insights", "查看用量洞察和分析", "Info",
               args_hint="[days]"),
    CommandDef("platforms", "查看 gateway/消息平台状态", "Info",
               cli_only=True, aliases=("gateway",)),
    CommandDef("platform", "暂停、恢复或列出异常 gateway platform", "Info",
               gateway_only=True, args_hint="<pause|resume|list> [name]"),
    CommandDef("copy", "复制最近的助手回复到剪贴板", "Info",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "附加剪贴板中的图片", "Info",
               cli_only=True),
    CommandDef("image", "为下一条 prompt 附加本地图片文件", "Info",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "更新 Hermes Agent 到最新版", "Info"),
    CommandDef("debug", "上传 debug 报告（系统信息和日志）并生成分享链接", "Info"),

    # Exit
    CommandDef("quit", "退出 CLI（使用 --delete 同时删除会话历史）", "Exit",
