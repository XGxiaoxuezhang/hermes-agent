...
def _get_platform_tools(
    ...
    *,
    ...
    def _toolset_tools_for_inference(ts_name: str) -> Set[str]:
        """Resolve built-in toolset membership without registry-only extras."""
        seen: Set[str] = set()

        def _walk(name: str) -> Set[str]:
            if name in seen:
                return set()
            seen.add(name)
            ...
            if not isinstance(ts_def, dict):
                return set()
            tools = {str(tool) for tool in ts_def.get("tools", [])}
            for child in ts_def.get("includes", []) or []:
                tools.update(_walk(str(child)))
            return tools

        inferred = _walk(ts_name)
        return inferred or set(resolve_toolset(ts_name))
        ...
        if composite_tools:
            ...
            for ts_key, _, _ in CONFIGURABLE_TOOLSETS:
                if not _toolset_allowed_for_platform(ts_key, platform):
                    continue
                ts_tools = _toolset_tools_for_inference(ts_key)
    ...
    for ck in configurable_keys:
        configurable_tool_universe.update(_toolset_tools_for_inference(ck))
    claimed = set()
    for ts_key in enabled_toolsets:
        claimed.update(_toolset_tools_for_inference(ts_key))
