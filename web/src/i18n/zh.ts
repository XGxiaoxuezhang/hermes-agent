...
export const zh: DeepPartial<Translations> = {
  ...
  models: {
    ...
    avgPerSession: "平均/会话",
    apiCalls: "API 调用",
    configureApiKeys: "配置 API Key",
    credentialsCtaBody:
      "先在这里选择主模型；如果提供商还没填 Key，直接进入密钥页粘贴即可，系统会写入 ~/.hermes/.env。",
    credentialsCtaTitle: "模型和密钥放到一起配置",
    modelSettings: "模型设置",
    appliesToNewSessions: "对新会话生效",
    showAuxiliary: "显示辅助模型",
    hideAuxiliary: "隐藏辅助模型",
    mainModel: "主模型",
    change: "更改",
    auxiliaryTasks: "辅助任务",
    resetAllToAuto: "全部恢复自动",
    resetAllConfirm: "确认把所有辅助任务恢复为自动？这会覆盖已经设置的单独任务模型。",
    auxiliaryDescription:
      "辅助任务用于图片分析、会话搜索、上下文压缩等后台工作。auto 表示使用主模型；需要便宜或更快的模型时可以单独覆盖。",
    autoUseMainModel: "auto（使用主模型）",
    providerDefault: "提供商默认模型",
    setMainModel: "设置主模型",
