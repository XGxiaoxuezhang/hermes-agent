...
import type { DeepPartial, Locale, Translations } from "./types";
import { en } from "./en";
import { zh } from "./zh";
  ...
  for (const [key, value] of Object.entries(override)) {
    ...
    merged[key] =
      isRecord(baseValue) && isRecord(value)
        ? mergeTranslations(baseValue, value as DeepPartial<typeof baseValue>)
        : value;
  }
  return merged as T;
}

const LOCALE_TRANSLATIONS: Record<Locale, DeepPartial<Translations>> = {
  en,
  zh,
  "zh-hant": zhHant,
  ja,
...
const TRANSLATIONS: Record<Locale, Translations> = Object.fromEntries(
  Object.entries(LOCALE_TRANSLATIONS).map(([locale, translations]) => [
    locale,
    locale === "en" ? en : mergeTranslations(en, translations),
  ]),
