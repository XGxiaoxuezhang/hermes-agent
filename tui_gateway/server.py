...
def _replace_session_agent_after_model_switch(sid: str, session: dict) -> None:
    """Rebuild the idle session agent after a model switch when needed."""
    old_agent = session.get("agent")
    session_key = str(session.get("session_key") or "").strip()
    if not session_key:
        raise ValueError("session_key required for agent rebuild")
    ...
    try:
        new_agent = _make_agent(
            ...
            reasoning_config_override=session.get("reasoning_config"),
            service_tier_override=getattr(old_agent, "service_tier", None)
            if old_agent is not None
            else None,
        )
    except TypeError:
        new_agent = _make_agent(sid, session_key, session_id=session_key)
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        yolo = False
    info: dict = {
        "session_key": getattr(agent, "session_id", "") or (session or {}).get("session_key", ""),
...
def _(rid, params: dict) -> dict:
    ...
    with session["history_lock"]:
        ...
        if removed:
            session["history_version"] = int(session.get("history_version", 0)) + 1
            rewritten_history = list(history)
    if removed and session.get("session_key"):
        try:
            db = _get_db()
            if db is not None and hasattr(db, "replace_messages"):
                db.replace_messages(session["session_key"], rewritten_history or [])
                agent = session.get("agent")
                if agent is not None and hasattr(agent, "_last_flushed_db_idx"):
                    agent._last_flushed_db_idx = len(rewritten_history or [])
        except Exception:
            logger.exception("session.undo failed to persist rewritten history")
